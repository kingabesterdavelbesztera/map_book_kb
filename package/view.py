...
def pokaz_szczegoly_uzytkownika():
    ...
    label_imie_szcegoly_obiektow_wartosc.config(text=imie)
    label_nazwisko_szcegoly_obiektow_wartosc.config(text=nazwisko)
    label_posty_szczegoly_obiektow_wartosc.config(text=posty)
    label_lokalizacja_szczegoly_obiektow_wartosc.config(text=lokalizacja)

def edytuj_użytkownika():
    i=listbox_lista_obiektow.index(ACTIVE)
...
def aktualizuj_uzytkownika(i):
    ...
    lista_uzytkownikow()
    button_dodaj_uzytkownika.config(text="Dodaj użytkownika: ", command=dodaj_uzytkownia)
    entry_imie.delete(0, END)
    entry_nazwisko.delete(0, END)
    entry_posty.delete(0,END)
    entry_lokalizacja.delete(0,END)
...
button_edytuj_obiekt = Button(ramka_lista_obiektow, text="Edytuj obiekt", command=edytuj_użytkownika)
...
button_dodaj_uzytkownika = Button(ramka_formularz, text="Dodaj użytkownika: ", command=dodaj_uzytkownia)
...
label_imie_szcegoly_obiektow_wartosc = Label(ramka_szczegoly_obiektow, text=".... ", width=10)
label_nazwisko_szcegoly_obiektow_wartosc = Label(ramka_szczegoly_obiektow, text=".... ", width=10)
label_posty_szczegoly_obiektow_wartosc = Label(ramka_szczegoly_obiektow, text=".... ", width=10)
label_lokalizacja_szczegoly_obiektow_wartosc = Label(ramka_szczegoly_obiektow, text=".... ", width=10)

label_szczegoly_obiektow.grid(row=0, column=0, sticky=W)
label_imie_szcegoly_obiektow.grid(row=1, column=0, sticky=W)
...
map_widget=tkintermapview.TkinterMapView(ramka_szczegoly_obiektow, width=900, height=400)
map_widget.set_position(52.2,21.0 )
map_widget.set_zoom(8)
marker_WAT = map_widget.set_marker(52.2546, 20.90025, text="WAT")
